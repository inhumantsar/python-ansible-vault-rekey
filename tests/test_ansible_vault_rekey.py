...
def test_rekey_file_withdecrypt():
    with open(join(PLAY, "group_vars/nosecrets.yml"), 'rb') as f:
        expected = f.read()
...
def test_backup_files():
    ...
    backedup_files_relpaths = [realpath(i)[len(realpath('.')) + 1:] for i in
                               rekey.backup_files(files, join(TMP_DIR, 'test_backup'))]
    for i in files:
...
def test_restore_files():
    ...
    found_files = [realpath(i)[len(realpath('.')) + 1:] for i in
                   rekey.find_files(join(TMP_DIR, 'test_restore'), pattern='*')]
    for i in files:
